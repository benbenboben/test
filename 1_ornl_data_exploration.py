...
nsrdb = cs_detection.ClearskyDetection.read_pickle('ornl_nsrdb.pkl.gz')


# In[3]:


ground = cs_detection.ClearskyDetection.read_pickle('ornl_ground.pkl.gz')


# In[4]:


nsrdb.trim_dates('01-01-2009', '01-01-2010')
...
nsrdb.robust_rolling_smooth('Clearsky GHI stat', 3)vis = visualize.Visualizer()vis.add_line_ser(nsrdb.df['GHI'], 'GHI')
vis.add_line_ser(nsrdb.df['Clearsky GHI'], 'Clearsky GHI')
vis.add_line_ser(nsrdb.df['Clearsky GHI pvlib'], 'Clearsky GHI pvlib')
vis.add_line_ser(nsrdb.df['Clearsky GHI stat'], 'Clearsky GHI stat')
vis.add_line_ser(nsrdb.df['Clearsky GHI stat smooth'], 'Clearsky GHI stat smooth')vis.show()
# ## Ground
...
ground.robust_rolling_smooth('Clearsky GHI stat', 60)vis = visualize.Visualizer()vis.add_line_ser(ground.df['GHI'], 'GHI')
vis.add_line_ser(ground.df['Clearsky GHI pvlib'], 'Clearsky GHI pvlib')
vis.add_line_ser(ground.df['Clearsky GHI stat'], 'Clearsky GHI stat')
vis.add_line_ser(ground.df['Clearsky GHI stat smooth'], 'Clearsky GHI stat smooth')
vis.add_circle_ser(ground.df[ground.df['sky_status pvlib'] == 1]['GHI'], 'pvlib clear')vis.show()utils.mean_abs_diff(ground.df['Clearsky GHI pvlib'], ground.df['Clearsky GHI stat smooth'])
...
nsrdb.robust_rolling_smooth('Clearsky GHI stat', 3)ground.robust_rolling_smooth('Clearsky GHI stat', 60)
# In[27]:


nsrdb.to_pickle('ornl_nsrdb_1.pkl.gz', overwrite=True)


# In[28]:


ground.to_pickle('ornl_ground_1.pkl.gz', overwrite=True)
